...
def is_world_cup_path(path: str) -> bool:
    query = parse_qs(urlsplit(path).query, keep_blank_values=True)
    return query.get("skuMode", [""])[0].strip().upper() == "WC"
...
def fifa14_client_config_values(
    ...
    *,
    ...
    if False and config_id == "OSDK_CLIENT" and returning_user:
        replacements = {
...
def main() -> int:
    ...
    if args.redirector_mode == "tls":
        ...
        redirector.main_blaze_host = args.host
    ...
    wc_identity_db = identity_db.with_name("wc-revival-worldcup.sqlite3")

    wc_identity_store = LocalIdentityStore(
        wc_identity_db,
        "new",
    )

    fut_http.identity_store = identity_store
    fut_http.wc_identity_store = wc_identity_store

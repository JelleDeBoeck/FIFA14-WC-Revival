  ...
  setteamid(teamId) {
    ...
    try {
      ...
      if (controller.isNull())
        return {ok:false, reason:'controller null'};

      const value = Number(teamId) >>> 0;

      // Same state proven to feed vtable +0x1F8 / TeamName_%d.
      controller.add(0x13148).writeU32(1);
      controller.add(0x13150).writeU32(value);

      return {
        ok:true,
        controller:controller.toString(),
        team_id:controller.add(0x13150).readU32()
      };
    } catch (error) {
...
def main() -> int:
    ...
    def read_wc_team_id() -> int | None:
        database = Path(r".\state\wc-revival-worldcup.sqlite3")
        if not database.is_file():
            return None
        ...
        try:
            with sqlite3.connect(database, timeout=0.1) as connection:
                ...
                identity = connection.execute(
                    "SELECT persona_id FROM identity WHERE singleton=1"
                ).fetchone()
    ...
    try:
        while time.monotonic() < deadline:
            ...
            current_wc_team_id = read_wc_team_id()

            if current_wc_team_id is not None:
                try:
                    ...
                    if isinstance(result, dict) and result.get("ok") and current_wc_team_id != last_wc_team_id:
                        ...
                        last_wc_team_id = current_wc_team_id

                except Exception as error:
                    record({
                        "kind": "cards-wc-teamname-publish-error",
                        "error": str(error),
                    })        
            time.sleep(0.25)
